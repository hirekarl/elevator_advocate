...
class BuildingViewSet(viewsets.ReadOnlyModelViewSet[Building]):
    ...
    def advocacy_summary(self, request, bin=None):
        """
        Returns the cached Executive Advocacy Summary for this building.

        Summaries are pre-generated by the generate_summaries management command
        (runs on every deploy). Returns 404 if no cache exists yet for the
        requested language — the frontend renders an empty state in that case.
        """
        building = self.get_object()
        ...
        if not cached:
            return Response(
                {
                    "error": "Summary not yet available. Run generate_summaries to populate."
                },
    ...
    def refresh_news(self, request, bin=None):
        """
        News is refreshed automatically on the 1st and 15th of each month
        via a Render cron job. On-demand refresh is disabled to protect the
        SerpAPI quota (250 searches/month).
        """
        building = self.get_object()
        return Response(
            {
                "message": f"News for {building.address} refreshes automatically on the 1st and 15th of each month.",
                "status": "SCHEDULED",
            }
        )
